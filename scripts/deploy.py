...
def run_capture(*args: list[str], **kwargs) -> str:
  proc = subprocess.run(args, check = True, stdout = subprocess.PIPE, stderr = None, **kwargs)
  return proc.stdout
...
@click.command()
@click.option("--rsync-target", help="Where bootstrap zipballs and source tarball will be copied to. If given, this is passed directly to rsync so it can be a local folder, ssh path, etc. For production builds this should be a webroot directory that will be served at bootstrap-url")
...
def deploy(public_url: str, rsync_target: str|None, bootstrap_url: str|None, arches: list[str]) -> None:
  ...
  for arch in arches:
    ...
    elif (old_attrs_text := attrs_file.read_text()) != attrs_text:
